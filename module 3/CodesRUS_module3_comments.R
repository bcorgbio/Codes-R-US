...
#CPK, can't load your data! Remember, by default, the code looks to the project directory! [-1]. Changed to work.
#load data
#anole <- read_csv("anole.dat.csv")
#anole.eco <- read_csv("anole.eco.csv")

anole <- read_csv("./module 3/anole.dat.csv")
anole.eco <- read_csv("./module 3/anole.eco.csv")
...
#CPK: Still lots of stuff you don't need. Below are examples from the descrption [-1]
...
#CPK:This paste of an example won't run bc you include a variable that isn't in this script: pgls.BM2. And you don't need it so commenting out.
...
#CPK: again, the path issue

anole.tree <- read.tree("./module 3/anole.tre")
...
#CPK: but you visualize results with ecomorph and this isn't included in models
...
#CPK. Well done! But,you say significant, but that only applies if you performed a test. You just assessed the fit of the model. Could be a good fit, but covar could be non-significant.For that . . .
...
#CPK. But again, the last prompt didn't ask about ecomorph, just to produce a plot of your own design that concisely visualizes the effect of your covariate(s) on the hindlimb residuals of the best fitting PGLS model. You'reassessing ecomorph, but it's not in your models! Would be ok if you did. In your plots, you regress the residuals against the covariates. Doesn' make sense. But, it was as simple as this.

anole.log%>%
  ggplot(aes(PHPD.res, HTotal))+geom_point()+geom_smooth(method="lm")

#CPK. Good work. Just be careful about file paths, not to include what we dont' need and address the prompts with the appropriate actions. [7/10]
